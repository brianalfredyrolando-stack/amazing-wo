...
import { tierPackages, allInInclusions, CONTACT } from "./paket/data";
import { formatRupiah } from "@/lib/format";
import AllInPricing from "./paket/AllInPricing";

export const metadata: Metadata = {
  title: "Amazing Concept Manado — Wedding Organizer & Paket Pernikahan",
  description:
    "Paket wedding organizer Amazing Concept Manado: Joyful, Wonderful, Luxurious, dan All in Package.",
};
  ...
  return (
    <main className="min-h-screen">
      ...
      <header className="relative flex min-h-[92vh] items-center justify-center overflow-hidden text-center text-ivory">
        ...
        {/* nav atas: wordmark + tombol masuk/dashboard */}
        <span className="absolute left-6 top-6 z-20 font-display text-lg tracking-wide text-ivory">
          Amazing WO
        </span>
        {session ? (
          <Link
            href="/dashboard"
            className="absolute right-5 top-5 z-20 inline-flex items-center gap-2 rounded-full bg-gold px-5 py-2 text-sm font-medium text-ivory transition hover:opacity-90"
          >
            Dashboard
            <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M5 12h14M13 6l6 6-6 6" /></svg>
          </Link>
        ) : (
          <Link
            href="/login"
            className="absolute right-5 top-5 z-20 inline-flex items-center gap-2 rounded-full border border-ivory/40 bg-ink/20 px-5 py-2 text-sm font-medium text-ivory backdrop-blur-sm transition hover:bg-ink/40"
          >
            <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24"><path d="M15 3h4a2 2 0 012 2v14a2 2 0 01-2 2h-4M10 17l5-5-5-5M15 12H3" /></svg>
            Masuk
          </Link>
        )}
